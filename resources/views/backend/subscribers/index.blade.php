                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header row">
                            <h3 class="card-title col-md-12">Subscribers: <a href="{{route('subscribers.create')}}" class="btn btn-success float-right">Add Subscriber</a></h3>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="data-table table border dataTable no-footer text-nowrap text-md-nowrap table-striped mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($subscribers as $detail)
                                            <tr>
                                                <td>{{$detail->id}}</td>
                                                <td>{{$detail['users']['name'].' ('.$detail['users']['email'].')'}}</td>
                                                <td>{{$detail->email}}</td>
                                                <td>{{ ($detail->status == 1) ? 'Active' : 'In Active' }}</td>
                                                <td>{{$detail->created_at}}</td>
                                                <td class="text-center align-middle">
                                                    <form action="{{ route('subscribers.destroy', $detail->id) }}" method="POST">
                                                        <div class="btn-group align-top btn-list">
                                                            <a class="btn btn-sm btn-primary badge" href="{{ route('subscribers.edit', $detail->id) }}"><i class="fa fa-edit"></i></a>
