                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header row">
                            <h3 class="card-title col-md-12">Tools: <a href="{{route('tools.create')}}" class="btn btn-success float-right">Add Tool</a></h3>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="data-table table border dataTable no-footer text-nowrap text-md-nowrap table-striped mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($tool as $detail)
                                            <tr>
                                                <td>{{$detail->id}}</td>
                                                <td>{{$detail->title}}</td>
                                                <td>{{$detail->slug}}</td>
                                                <td>{{ ($detail->status == "0") ? "In Active" : "Active" }}</td>
                                                <td><img src="{{asset('assets/images/tools/'.$detail->feature)}}" width="50px"></td>
                                                <td>{{$detail['categories']['title']}}</td>
                                                <td>{{$detail->created_at}}</td>
                                                <td class="text-center align-middle">
                                                    <form action="{{ route('tools.destroy', $detail->id) }}" method="POST">
                                                        <div class="btn-group align-top btn-list">
                                                            <a class="btn btn-sm btn-primary badge" href="{{ route('tools.edit', $detail->id) }}"><i class="fa fa-edit"></i></a>
